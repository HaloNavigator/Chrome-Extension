...
export interface ThemeConfig {
  ...
  actionReveal?: boolean;
  faviconTinting?: boolean;
}
...
const ACTIONS_KEY = 'halo_navigator_actions';
...
const App: React.FC = () => {
  ...
  const [rules, setRules] = useState<NavigatorRule[]>([
    { id: '1', pattern: 'tenant.haloitsm.com', color: '#ef4444', label: 'Prod', labelColor: '#ffffff', styleType: 'full', hideLabel: false, labelPosition: 'right', strength: 5 },
    { id: '2', pattern: 'dev.haloitsm.com', color: '#22c55e', label: 'Dev', labelColor: '#ffffff', styleType: 'border', hideLabel: false, labelPosition: 'left', strength: 8 },
    { id: '3', pattern: 'uat.haloitsm.com', color: '#f59e0b', label: 'UAT', labelColor: '#ffffff', styleType: 'top-bar', hideLabel: false, labelPosition: 'center', strength: 4 },
  ]);
  ...
  const handleHaloAction = useCallback((path: string) => {
    if (!path.startsWith('action:')) return false;
    
    const actionId = path.replace('action:', '');
    ...
    if (action) {
      ...
      showBanner(`Action Triggered: ${action.name} (Target: ${targetId})`);
      return true;
    } else {
      showBanner("Action Protocol Error", "error");
      return false;
    }
  }, [actions]);

  // Rain Effect Logic - Explicitly 10 Seconds
  useEffect(() => {
  ...
  useEffect(() => { if (activeEffect !== 'lopan') setIdleBeams(false); }, [activeEffect]);
  ...
  const saveSettings = useCallback(() => {
    ...
    n.className = "fixed bottom-4 left-1/2 -translate-x-1/2 bg-emerald-600 text-white px-4 py-2 rounded-full text-xs font-bold shadow-lg z-50 animate-bounce";
    n.innerText = "Applied & Saved!";
    document.body.appendChild(n);
    setTimeout(() => n.remove(), 2000);
  }, [rules, commands, actions, themeConfig, vault]);

  const exportData = useCallback(() => {
    const data = { rules, commands, actions, themeConfig, vault, exportedAt: new Date().toISOString(), version: "1.1" };
  ...
  const updateActions = (newActions: CustomAction[]) => setActions(newActions);
  ...
  const handleFormatSql = () => showBanner("SQL Prettified");
  ...
  const handlePaletteExecute = (path: string, openInNewTab: boolean) => {
    if (path.startsWith('action:')) { handleHaloAction(path); return; }
    ...
    if (path.startsWith('effect:')) {
      ...
      if (effectName === 'checkisinthemail') showBanner("Yessir, the check is in the mail.");
      if (effectName === 'duespaid') showBanner("It's all in the reflexes");
      if (effectName === 'sixdemonbag') {
          setTimeout(() => {
              const n = document.createElement('div');
      ...
      if (effectName === 'lopan') setIdleBeams(true);
      if (effectName === 'lightning') {
          const seq = [100, 300, 100, 500, 100, 200, 100, 400, 100, 600];
          let acc = 0;
          seq.forEach((delay) => {
              acc += delay; if (acc > 3000) return;
              setTimeout(() => {
      ...
      // ENSURED 10S DURATION FOR RAIN AND OTHERS
      const duration = (effectName === 'thunder' || effectName === 'lightning') ? 3000 : 10000;
      setTimeout(() => setActiveEffect(null), duration);
      return;
    }
    let finalUrl = path;
    if (!path.includes('://')) {
      const baseUrl = tabs.find(t => t.id === activeTabId)?.url.split('/')[0] || 'tenant.haloitsm.com';
  ...
  return (
    <div className={`h-screen flex flex-col items-center p-4 md:p-6 lg:p-8 transition-all duration-500 font-sans ${isDarkMode ? 'bg-[#0f172a]' : 'bg-slate-50'} ${activeEffect === 'duespaid' ? 'dues-active' : ''} ${activeEffect === 'lopan' ? 'lopan-active' : ''} ${activeEffect === 'thunder' ? 'thunder-active' : ''} ${activeEffect === 'checkisinthemail' ? 'jack-burton-active' : ''}`}>
      <style>{`
        ...
        @keyframes shake { 0% { transform: translate(5px, 5px) rotate(0deg); } 10% { transform: translate(-5px, -8px) rotate(-1deg); } 20% { transform: translate(-10px, 0px) rotate(1deg); } 30% { transform: translate(10px, 8px) rotate(0deg); } 40% { transform: translate(5px, -5px) rotate(1deg); } 50% { transform: translate(-5px, 8px) rotate(-1deg); } 60% { transform: translate(-10px, 5px) rotate(0deg); } 70% { transform: translate(10px, 5px) rotate(-1deg); } 80% { transform: translate(-5px, -5px) rotate(1deg); } 90% { transform: translate(5px, 8px) rotate(0deg); } 100% { transform: translate(5px, -8px) rotate(-1deg); } }
        ...
        .lopan-beam { position: fixed; top: 0; width: 80px; height: 100vh; background: linear-gradient(to bottom, ${themeConfig.primaryColor}, transparent); filter: blur(40px); opacity: 0.6; z-index: 100000; animation: beamPulse 1.5s infinite ease-in-out; }
        @keyframes beamPulse { 0%, 100% { opacity: 0.3; transform: scaleX(1); } 50% { opacity: 0.8; transform: scaleX(1.2); } }
        .porkchop-banner { position: fixed; top: 0; left: 0; right: 0; background: #b91c1c; color: white; font-family: 'Impact', sans-serif; text-transform: uppercase; padding: 12px; text-align: center; font-size: 14px; letter-spacing: 0.3em; z-index: 2147483647; border-bottom: 4px solid black; box-shadow: 0 10px 30px rgba(0,0,0,0.5); animation: slideDown 0.4s cubic-bezier(0.16, 1, 0.3, 1) forwards; }
        @keyframes slideDown { from { transform: translateY(-100%); } to { transform: translateY(0); } }
      `}</style>
      {activeEffect === 'checkisinthemail' && <div className="porkchop-banner">Porkchop Express — Hauling Solutions</div>}
      {activeEffect === 'rain' && <canvas ref={rainCanvasRef} className="fixed inset-0 pointer-events-none z-[9999]" />}
      {activeEffect === 'navigator-matrix' && <canvas ref={matrixCanvasRef} className="fixed inset-0 pointer-events-none z-[9999]" style={{ opacity: 0.4 }} />}
      {idleBeams && (<><div className="lopan-beam" style={{ left: '15%' }} /><div className="lopan-beam" style={{ right: '15%' }} /></>)}
      {bannerText && (
        <div className="fixed top-8 left-1/2 -translate-x-1/2 z-[100001] pointer-events-none banner-in">
          <div className={`px-8 py-4 rounded-[2rem] border-2 shadow-2xl flex items-center gap-4 transition-colors duration-500 ${isDarkMode ? 'bg-[#1e293b] border-opacity-50' : 'bg-white border-opacity-50'}`} style={{ borderColor: themeConfig.primaryColor, boxShadow: `0 25px 50px -12px rgba(0,0,0,0.5), 0 0 30px ${themeConfig.primaryColor}33` }}>
            <div className="w-10 h-10 rounded-2xl flex items-center justify-center animate-pulse" style={{ backgroundColor: `${themeConfig.primaryColor}15`, color: themeConfig.primaryColor }}><ShieldCheckIcon className="w-6 h-6" /></div>
            <span className={`text-sm font-black uppercase tracking-widest ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{bannerText}</span>
          </div>
        </div>
      )}
      <CommandPalette isOpen={isPaletteOpen} onClose={() => setIsPaletteOpen(false)} commands={sortedCommands} actions={actions} onExecute={handlePaletteExecute} themeConfig={themeConfig} vault={vault} />
      <div className="mb-4 text-center max-w-2xl shrink-0">
        <div className={`text-2xl font-bold mb-1 flex items-center justify-center gap-3 transition-colors ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
          <img src="images/HN48.png" className="w-8 h-8 shadow-xl rounded-lg" alt="Logo" /><h1 style={{ color: !isDarkMode ? '#1e293b' : 'white' }}>Halo Navigator</h1>
        </div>
        <p className={`text-xs transition-colors ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>Professional environment visualization for HaloITSM.</p>
      </div>
      <div className={`relative w-full max-w-[1800px] flex-1 flex flex-col lg:flex-row gap-6 min-h-0 ${activeEffect === 'flip' ? 'flip-active' : ''}`}>
        <div className={`flex-1 flex flex-col rounded-[2.5rem] shadow-2xl border transition-all duration-500 overflow-hidden min-h-0 ${isDarkMode ? 'bg-[#0f172a] border-white/5' : 'bg-white border-slate-200'} ${activeEffect === 'ghost' ? 'ghost-active' : ''}`}>
          <BrowserHeader tabs={tabs} activeTabId={activeTabId} onSetActiveTab={setActiveTabId} onAddTab={() => addTab()} onCloseTab={closeTab} onUrlChange={updateActiveTabUrl} onExtensionClick={() => setIsExtensionOpen(!isExtensionOpen)} isExtensionOpen={isExtensionOpen} rules={rules} isDarkMode={isDarkMode} toggleDarkMode={() => setThemeConfig(prev => ({ ...prev, mode: prev.mode === 'dark' ? 'light' : 'dark' }))} themeConfig={themeConfig} />
          <div className="flex-1 flex flex-col min-h-0 overflow-hidden relative">
            {activeTab.type === 'settings' ? (
              <OptionsDashboard 
                ...
                onImport={importData} 
              />
            ) : activeTab.type === 'sql' ? (<SqlGeneratorView themeConfig={themeConfig} />) : (<SimulatedPage url={activeTab.url} matchedRule={matchedRule} themeConfig={themeConfig} commands={sortedCommands} onNavigate={updateActiveTabUrl} onOpenNewTab={(url) => addTab(url, 'Module Redirect')} formData={formData} onInputChange={handleInputChange} onAddTab={addTab} />)}
          </div>
          <div className={`border-t px-8 py-2 flex items-center gap-8 justify-center text-[9px] font-bold uppercase tracking-[0.2em] shrink-0 transition-colors ${isDarkMode ? 'bg-[#0f172a] border-white/5 text-slate-500' : 'bg-slate-50 border-slate-200 text-slate-400'}`}>
            <div className="flex items-center gap-1.5 cursor-pointer hover:text-indigo-400 transition-colors" onClick={() => setIsPaletteOpen(true)}><span className="bg-[#1e293b] dark:bg-[#1a1f2e] px-1.5 py-0.5 rounded text-indigo-500 font-black">#</span><span>Navigator Palette</span></div>
            <div className="flex items-center gap-1.5"><CommandIcon className="w-3.5 h-3.5" /><span>TAB <span className="font-normal lowercase text-[8px]">New Tab</span></span></div>
          </div>
        </div>
        {isExtensionOpen && (
          <div className="w-full lg:w-[480px] flex flex-col min-h-0 animate-in fade-in slide-in-from-right-8 duration-500">
            <div className={`flex-1 flex flex-col rounded-[2.5rem] shadow-[0_35px_60px_-15px_rgba(0,0,0,0.3)] border overflow-hidden transition-all duration-500 min-h-0 ${isDarkMode ? 'bg-[#0f172a] border-white/5 ring-4 ring-[#00ff87]/5' : 'bg-white border-slate-200 ring-4 ring-[#00ff87]/10'}`}>
              <ExtensionPopup rules={rules} onAdd={addRule} onRemove={removeRule} onUpdate={updateRule} onSave={saveSettings} themeConfig={themeConfig} onUpdateTheme={setThemeConfig} commands={sortedCommands} onUpdateCommands={updateCommands} actions={actions} onUpdateActions={updateActions} vault={vault} onSaveVault={handleSaveToVault} onUpdateVaultName={handleUpdateVaultName} onApplyVault={handleApplyVault} onDeleteVault={handleDeleteVault} onOpenOptions={() => addTab('halo-navigator://settings', 'Halo Navigator Hub', 'settings')} onOpenSqlGen={() => addTab('halo-navigator://sql', 'SQL Query Generator', 'sql')} onMagicFill={handleMagicFill} onFormatSql={handleFormatSql} onExport={exportData} onImport={importData} />
            </div>
          </div>
        )}
      </div>
