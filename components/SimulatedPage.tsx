...
export const SimulatedPage: React.FC<SimulatedPageProps> = ({ 
  url, matchedRule, themeConfig, formData, onInputChange, onOpenNewTab, onAddTab
}) => {
  ...
  const ActionRevealBadge = ({ id }: { id: string }) => {
    if (!themeConfig.actionReveal) return null;
    return (
      <span 
        onClick={(e) => {
          e.preventDefault();
          e.stopPropagation();
          onAddTab(`/config/tickets/outcomes?id=${id}`, `Outcome ${id}`);
        }}
        className="inline-flex items-center ml-2 px-2 py-0.5 rounded-[6px] text-[10px] font-black uppercase tracking-tight cursor-pointer transition-all hover:scale-110 active:scale-95"
        style={{ 
          backgroundColor: `${accent}15`, 
        ...
        title={`Configure Action Outcome ${id}`}
      >
        ACT_{id}
      </span>
    );
  ...
  return (
    <div className={`flex-1 p-0 transition-all duration-500 flex flex-col relative overflow-hidden ${isDarkMode ? 'bg-[#0f172a]' : 'bg-[#f4f7fa]'}`}>
      ...
      {matchedRule && !matchedRule.hideLabel && (
        ...
        >
          <span 
            className="text-[11px] font-black uppercase tracking-[0.2em] select-none"
            style={{ 
              color: matchedRule.labelColor || '#121212',
      ...
      <div className="z-10 w-full shrink-0">
          <div className="bg-[#f97316] text-white px-8 py-5 rounded-tr-[2rem] flex items-center justify-between shadow-lg">
             ...
             <div className="relative group">
               <button className="bg-black/20 text-white px-6 py-2 rounded-full text-xs font-bold uppercase tracking-wider hover:bg-black/30 transition-all">
                 Actions
               </button>
               <div className="menu transition opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto absolute right-0 top-full mt-2 w-64 bg-[#1e293b] border border-white/10 rounded-2xl shadow-2xl p-2 z-50">
                 <div className="flex items-center justify-between px-3 py-2 text-xs font-bold text-white/40 uppercase tracking-widest">
                   <span>Outcomes</span>
                 </div>
                 <a href="#" className="flex items-center justify-between px-3 py-2.5 rounded-lg hover:bg-white/5 text-white text-sm transition-colors">
                   <span>Create Change Request</span>
                   <ActionRevealBadge id="158" />
                 </a>
                 <a href="#" className="flex items-center justify-between px-3 py-2.5 rounded-lg hover:bg-white/5 text-white text-sm transition-colors">
                   <span>Create Task</span>
                   <ActionRevealBadge id="83" />
                 </a>
               </div>
             </div>
          </div>
