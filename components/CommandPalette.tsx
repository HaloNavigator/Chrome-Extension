...
import { CommandMapping, ThemeConfig, VaultEntry, CustomAction } from '../App';
...
export const CommandPalette: React.FC<CommandPaletteProps> = ({ 
  isOpen, onClose, commands, actions, onExecute, themeConfig, vault
}) => {
  ...
  const filteredCommands = useMemo(() => {
    ...
    // INTERCEPT: If input starts with "#", we prioritize custom actions
    if (search.startsWith('#') && search.length > 1) {
        const matches = actions.filter(a => 
            a.shortcut.toLowerCase().includes(search) ||
            a.name.toLowerCase().includes(search.substring(1))
        );
        return matches.map(a => ({
            code: a.shortcut.toUpperCase(),
            path: `action:${a.id}`,
            description: `Trigger Action ➔ ${a.name} (${a.type})`,
            type: 'custom-action'
        }));
    }

    // PASS-BACK: Standard logic for jumps, effects, and protocols
    ...
    if (/^cap\s+/i.test(input)) {
        ...
        const matches = vault.filter(v => v.name.toLowerCase().includes(subSearch));
  ...
  }, [input, commands, vault, actions]);

  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Tab') {
      e.preventDefault();
      const target = filteredCommands[selectedIndex];
      if (target) { onExecute(target.path, true); onClose(); }
      return;
    }
    ...
    if (e.key === 'Enter') {
      e.preventDefault();
      const search = input.toLowerCase().trim();
      if (search === 'fill') { onExecute('magic-fill', false); onClose(); return; } 
      if (search === 'cap') { onExecute('vault:capture', false); onClose(); return; }
      if (search === 'pretty') { onExecute('format-sql', false); onClose(); return; }

      const target = filteredCommands[selectedIndex];
      if (target) { onExecute(target.path, false); onClose(); }
      return;
    }
  ...
  return (
    <div className="fixed inset-0 z-[10000] flex items-start justify-center pt-[15vh] px-4 animate-in fade-in duration-300">
      ...
      <div 
        ...
        style={{ 
          backgroundColor: paletteBg, borderColor: borderColor,
      ...
      >
        ...
        <div className="flex items-center px-10 py-8 border-b" style={{ borderColor: borderColor }}>
          <div className="mr-8"><span className="text-3xl font-black italic tracking-tighter" style={{ color: accent }}>#</span></div>
          <input
            ref={inputRef} type="text" value={input}
            onChange={(e) => { setInput(e.target.value); setSelectedIndex(0); }}
            onKeyDown={handleKeyDown} placeholder="Search Protocol..."
        ...
        <div className="px-10 py-3 bg-black/5 border-b flex gap-4 overflow-x-auto no-scrollbar" style={{ borderColor: borderColor }}>
          <div className="flex items-center gap-1.5 shrink-0">
            <kbd className="px-1.5 py-0.5 rounded bg-black/40 text-[9px] font-black border border-white/10 text-white"># [NUM]</kbd>
            <span className="text-[9px] font-black uppercase tracking-widest opacity-40">Actions</span>
          </div>
          <div className="flex items-center gap-1.5 shrink-0">
        ...
        <div className="max-h-[380px] overflow-y-auto no-scrollbar p-4 space-y-2">
          {filteredCommands.map((cmd, idx) => {
            ...
            const isAction = (cmd.path || '').startsWith('action:');
            const isFill = cmd.path === 'magic-fill' || cmd.path === 'format-sql';
            ...
            return (
              <div
                ...
                style={{ backgroundColor: isSelected ? itemSelectedBg : 'transparent', borderColor: isSelected ? `${accent}77` : 'transparent' }}
              >
                <div className={`min-w-[64px] flex items-center justify-center font-black text-sm tracking-[0.1em] transition-all duration-200 ${isSelected ? 'scale-110' : 'opacity-80'}`} style={{ color: isSelected ? accent : textColor }}>
                  {isVault ? '🔒' : isAction ? '⚡' : isFill ? '✨' : isEffect ? '✨' : `#${cmd.code.toUpperCase()}`}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-[14px] font-black uppercase tracking-wide flex items-center gap-2" style={{ color: textColor }}>
                     ...
                     {!isEffect && !isVault && !isAction && !isFill && cmd.path.includes('://') && <GlobeIcon className="w-3.5 h-3.5 opacity-60" />}
                  </div>
                  <div className="text-[11px] truncate font-mono mt-1 tracking-tight" style={{ color: pathColor }}>{cmd.path}</div>
                </div>
                {isSelected && (<div className="flex items-center gap-3 animate-in slide-in-from-right-3 duration-300"><kbd className="px-3 py-1 rounded-lg bg-black/60 text-[9px] font-black border border-white/20 text-white">ENTER</kbd></div>)}
              </div>
            );
          })}
          {filteredCommands.length === 0 && (<div className="py-12 text-center opacity-30 text-[12px] font-black uppercase tracking-widest">Protocol Not Found</div>)}
        </div>
        ...
        <div className="px-10 py-6 border-t font-black uppercase tracking-[0.15em] text-[10px]" style={{ backgroundColor: footerBg, borderColor: borderColor, color: textColor }}>
           <div className="flex justify-between mb-1.5"><span>ENTER ↵ OPEN / TRIGGER</span><span>↗ NEW TAB (TAB)</span></div>
           <div className="flex justify-between opacity-50"><span>CAP + SEARCH</span><span># + NUMBER</span></div>
        </div>
      </div>
    </div>
  );
