...
const injectContent = async (tabId, rule, commands, themeConfig) => {
  try {
    ...
    await chrome.scripting.executeScript({
      target: { tabId: tabId },
      func: (activeRule, cmdList, theme, vaultEntries) => {
        ...
        const setupCommandPalette = () => {
          ...
          chrome.runtime.onMessage.addListener((request) => {
            if (request.type === 'VAULT_UPDATED' && request.vault) {
              window.__HALO_NAV_STATE__.vault = request.vault;
            }
            if (request.type === 'SHOW_BANNER') {
              showHaloBanner(request.message, request.bannerType || 'success');
            }
          });
            ...
            if (isHash && !isInput) { 
              ...
              palette.innerHTML = `
                ...
                <div style="padding:10px 32px; background:rgba(0,0,0,0.05); border-bottom:1px solid ${isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)'}; display:flex; gap:20px; overflow-x:auto;">
                  ...
                  <div style="display:flex; align-items:center; white-space:nowrap;"><kbd style="${kbStyle}">NT </kbd><span style="${guideLabelStyle}">Module Tab</span></div>
                  ...
                  <div style="display:flex; align-items:center; white-space:nowrap;"><kbd style="${kbStyle}">CAP </kbd><span style="${guideLabelStyle}">Inject</span></div>
                </div>
              ...
              const execute = (cmd, newTab = false) => {
                ...
                if (cmd.path === 'magic-fill') {
                ...
                } else if (cmd.path === 'vault:capture') {
                   chrome.runtime.sendMessage({ type: 'CAPTURE_FORM' });
                } else if (cmd.path === 'format-sql') {
                   chrome.runtime.sendMessage({ type: 'FORMAT_SQL' });
                   ...
                   const btn = document.querySelector('button.haloCircleBtn[title="New Tab"], button[title="New Tab"], button[aria-label="New Tab"], .crumbs[title="New Tab"] a, .fa-plus')?.closest('button') 
                              || document.querySelector('.crumbs[title="New Tab"] a');
                   if (btn) {
                       btn.click();
                       ...
                       if (parts.length > 2) {
                           ...
                           setTimeout(() => {
                               if (cleanPath.startsWith('http')) window.location.href = cleanPath;
                               else { window.history.pushState({}, '', cleanPath); window.dispatchEvent(new PopStateEvent('popstate')); }
                           }, 250);
                       }
                   }
                   ...
                   const cleanup = () => {
                       ...
                       const s = document.getElementById('halo-effect-styles');
                       if (s) s.innerHTML = '';
                   ...
                   if (effect === 'clean') {
                       ...
                       cleanup();
                       if (!document.getElementById('halo-effect-styles')) {
                           const s = document.createElement('style'); s.id = 'halo-effect-styles'; document.head.appendChild(s);
                       }
                       document.getElementById('halo-effect-styles').innerHTML += `
                           .halo-lopan-active { box-shadow: inset 0 0 150px ${primary}66 !important; transition: box-shadow 2s !important; }
                           @keyframes lopanBeams { 0% { height: 0; opacity: 0; } 50% { height: 100vh; opacity: 0.9; } 100% { height: 0; opacity: 0; } }
                           .lopan-beam { position: fixed; top: 0; width: 60px; background: linear-gradient(to bottom, ${primary}, rgba(0,255,135,0)); z-index: 2147483640; filter: blur(20px); pointer-events: none; animation: lopanBeams 4s infinite ease-in-out; }
                       `;
                       document.body.classList.add('halo-lopan-active');
                       const beams = document.createElement('div');
                       beams.id = 'halo-lopan-beams';
                       beams.innerHTML = `<div class="lopan-beam" style="left: 45%;"></div><div class="lopan-beam" style="right: 45%;"></div>`;
                       ...
                       document.getElementById('halo-effect-styles').innerHTML += `
                           @keyframes haloThunderShake { 0% { transform: translate(0,0); } 10% { transform: translate(-10px,-10px); } 20% { transform: translate(10px,10px); } 30% { transform: translate(-10px,10px); } 40% { transform: translate(10px,-10px); } 100% { transform: translate(0,0); } }
                           .halo-thunder-active { animation: haloThunderShake 0.4s infinite cubic-bezier(.36,.07,.19,.97) both; }
                       `;
                       ...
                       canvas.id = 'halo-rain-canvas';
                       canvas.style.cssText = 'position:fixed; inset:0; z-index:2147483640; pointer-events:none;';
                       ...
                       const rain = [];
                       for(let i=0; i<150; i++) {
                           rain.push({ x: Math.random()*canvas.width, y: Math.random()*canvas.height, l: Math.random()*25+15, s: Math.random()*20+15 });
                       }
                       const draw = () => {
                           ...
                           rain.forEach(p => {
                               ctx.beginPath();
                               ctx.moveTo(p.x, p.y);
                               ctx.lineTo(p.x + (p.s*0.1), p.y + p.l);
                               ctx.stroke();
                               p.y += p.s;
                               p.x += p.s*0.1;
                           ...
                           ctx.globalAlpha = 1.0;
                           window.__HALO_RAIN_ANIM__ = requestAnimationFrame(draw);
                       };
                       draw();
                       setTimeout(() => cleanup(), 10000); 
                   } else if (effect === 'lightning') {
                       const flash = document.createElement('div');
                       flash.id = 'halo-lightning-flash';
                       flash.style.cssText = 'position:fixed; inset:0; background:white; z-index:2147483645; pointer-events:none; opacity:0; transition: opacity 0.05s;';
                       document.body.appendChild(flash);
                       let startTime = Date.now();
                       const triggerStorm = () => {
                           if (Date.now() - startTime > 3000) { flash.remove(); return; }
                           flash.style.opacity = Math.random() > 0.5 ? '1.0' : '0.6';
                           setTimeout(() => {
                               flash.style.opacity = '0';
                               setTimeout(triggerStorm, Math.random() * 400 + 100);
                           }, 80);
                       };
                       triggerStorm();
                       ...
                       toast.id = 'halo-sixdemon-toast';
                       toast.style.cssText = `position:fixed; bottom:40px; left:50%; transform:translateX(-50%); background:#1e293b; color:#818cf8; padding:25px 50px; border-radius:30px; border:4px solid #818cf8; z-index:2147483647; font-weight:900; font-family:serif; font-style:italic; box-shadow:0 0 60px #818cf888; display:flex; flex-direction:column; align-items:center; gap:8px; animation: haloSlideDown 0.5s ease-out forwards;`;
                       ...
                       document.getElementById('halo-effect-styles').innerHTML += `
                           .halo-jack-burton-filter { filter: sepia(0.3) contrast(1.1) brightness(0.95) !important; }
                           @keyframes truckSlide { from { transform: translateY(-100%); } to { transform: translateY(0); } }
                       ...
                       const banner = document.createElement('div');
                       banner.id = 'halo-trucker-banner';
                       banner.innerText = "PORKCHOP EXPRESS — HAULING SOLUTIONS";
                       document.body.appendChild(banner);
                       ...
                       document.getElementById('halo-effect-styles').innerHTML += `
                           .halo-dues-active { filter: contrast(0.85) sepia(0.2) brightness(0.9) !important; font-family: 'Courier New', Courier, monospace !important; }
                           .halo-dues-active * { font-family: inherit !important; }
                           .halo-dues-active::before { content: ""; position: fixed; inset: 0; z-index: 2147483630; pointer-events: none; opacity: 0.05; background-image: url("data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.get-icons.com'%3E%3Cfilter id='noiseFilter'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.65' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noiseFilter)'/%3E%3C/svg%3E"); }
                       `;
                       ...
                       document.getElementById('halo-effect-styles').innerHTML += `
                           @keyframes haloGhostGlow { 0%, 100% { box-shadow: inset 0 0 30px #3b82f6; } 50% { box-shadow: inset 0 0 80px #3b82f6aa; } }
                           .halo-ghost-active { opacity: 0.6 !important; animation: haloGhostGlow 4s infinite ease-in-out !important; transition: opacity 1s, box-shadow 1s !important; }
                       `;
                       ...
                       overlay.id = 'halo-party-overlay';
                       overlay.style.cssText = 'position:fixed; inset:0; pointer-events:none; z-index:1000000;';
                       document.body.appendChild(overlay);
                       for(let i=0; i<100; i++) {
                           const c = document.createElement('div');
                           c.style.cssText = `position:absolute; top:-20px; left:${Math.random()*100}%; width:${Math.random()*10+5}px; height:${Math.random()*10+5}px; background:hsl(${Math.random()*360},70%,50%); border-radius:2px; transform:rotate(${Math.random()*360}deg); animation: haloConfetti ${Math.random()*3+2}s linear forwards;`;
                           overlay.appendChild(c);
                       }
                       if (!document.getElementById('halo-confetti-styles')) {
                           const s = document.createElement('style'); s.id = 'halo-confetti-styles'; s.innerHTML = `@keyframes haloConfetti { to { transform: translateY(105vh) rotate(720deg); opacity: 0; } }`; document.head.appendChild(s);
                       }
                       setTimeout(() => overlay.remove(), 10000);
                   } else if (effect === 'navigator-matrix') {
                       cleanup();
                       const canvas = document.createElement('canvas');
                       canvas.id = 'halo-matrix-canvas';
                       canvas.style.cssText = 'position:fixed; inset:0; z-index:999999; pointer-events:none; opacity:0.3; background:rgba(0,0,0,0.8);';
                       document.body.appendChild(canvas);
                       const ctx = canvas.getContext('2d');
                       canvas.width = window.innerWidth; canvas.height = window.innerHeight;
                       const chars = "HALOITSMNAVIGATOR01".split("");
                       const fontSize = 14; const columns = canvas.width/fontSize;
                       const drops = Array(Math.floor(columns)).fill(1);
                       const draw = () => {
                           ctx.fillStyle = "rgba(0, 0, 0, 0.05)"; ctx.fillRect(0, 0, canvas.width, canvas.height);
                           // Match Theme Accent Color
                           ctx.fillStyle = primary; ctx.font = fontSize + "px monospace";
              ...
              const render = () => {
                ...
                const search = rawVal.toLowerCase().trim();
                let resultsData = [];
                const state = window.__HALO_NAV_STATE__;
                const cmdListLive = state.commands || [];
                const vaultEntriesLive = state.vault || [];

                if (/^nt\s+/i.test(rawVal)) {
                    const subSearch = rawVal.substring(3).toLowerCase().trim();
                    resultsData = cmdListLive.filter(c => c.code.toLowerCase().includes(subSearch) || c.path.toLowerCase().includes(subSearch))
                        .map(c => ({ code: `NT ${c.code.toUpperCase()}`, path: `effect:new-halo-tab:${c.path}`, desc: `New Tab Context ➔ ${c.path}`, type: 'effect' })).slice(0, 10);
                    if (resultsData.length === 0 && subSearch === '') resultsData = [{ code: 'NT', path: 'effect:new-halo-tab', desc: 'Open Blank New Tab' }];
                } else if (/^cap\s+/i.test(rawVal)) {
                    const subSearch = rawVal.substring(4).toLowerCase().trim();
                    resultsData = vaultEntriesLive.filter(v => v.name.toLowerCase().includes(subSearch))
                        .map(v => ({ code: 'INJECT', path: `vault:apply:${v.id}`, desc: `Inject Snapshot ➔ ${v.name}`, type: 'vault' })).slice(0, 10);
                } else {
                    const effects = [
                       { code: 'CAP', path: 'vault:capture', desc: 'Capture Secure Form' },
                       { code: 'NT', path: 'effect:new-halo-tab', desc: 'Open New Halo Tab' },
                       { code: 'HALONAVIGATOR', path: 'effect:navigator-matrix', desc: 'Halo Navigator: Matrix Protocol' },
                       { code: 'HALO', path: 'effect:halo', desc: 'Spin Effect' },
                       { code: 'PARTY', path: 'effect:party', desc: 'Confetti' },
                       { code: 'LOPAN', path: 'effect:lopan', desc: 'David Lo Pan' }, 
                       { code: 'THUNDER', path: 'effect:thunder', desc: 'Thunder' },
                       { code: 'RAIN', path: 'effect:rain', desc: 'Rain' },
                       { code: 'LIGHTNING', path: 'effect:lightning', desc: 'Lightning' },
                       { code: 'MAIL', path: 'effect:checkisinthemail', desc: 'The Jack Burton Protocol' },
                       { code: 'SIXDEMONBAG', path: 'effect:sixdemonbag', desc: 'Six Demon Bag' },
                       { code: 'DUESPAID', path: 'effect:duespaid', desc: 'Dirty Tank Top Filter' },
                       { code: 'GHOST', path: 'effect:ghost', desc: 'Ghost mode' },
                       { code: 'CLEAN', path: 'effect:clean', desc: 'Clear All Effects' }
                    ];
                    const allOptions = [...cmdListLive, ...effects];
                    resultsData = allOptions.map(c => {
                      let score = 0; const code = (c.code || '').toLowerCase(); const path = (c.path || '').toLowerCase();
                      if (code === search) score += 1000; else if (code.startsWith(search)) score += 500; else if (code.includes(search)) score += 100;
                      if (path.startsWith(search) || path.startsWith('/' + search)) score += 50; else if (path.includes(search)) score += 10;
                      return { ...c, score };
                ...
                results.innerHTML = currentFiltered.map((c, i) => {
                  ...
                  const isEffect = (c.path || '').startsWith('effect:');
                  const isVault = (c.path || '').startsWith('vault:');
                  const isInject = c.code === 'INJECT';
                  return `<div data-index="${i}" class="halo-result-item" style="padding:16px 24px; border-radius:24px; display:flex; align-items:center; cursor:pointer; background:${isSelected ? primary + '33' : 'transparent'}; border: 1px solid ${isSelected ? primary + '66' : 'transparent'}; transition: 0.1s; margin-bottom: 4px;">
                      <span style="font-weight:900; font-size:13px; color:${itemColor}; min-width:110px;">${isVault || isInject ? '🔒' : isEffect ? (c.code.startsWith('NT') ? '➕' : '✨') : '#' + c.code.toUpperCase()}</span>
              ...
              results.onclick = (e) => {
                ...
                if (item) { const index = parseInt(item.getAttribute('data-index')); execute(currentFiltered[index], e.ctrlKey || e.metaKey); }
              };
              results.onmousemove = (e) => {
                if (e.clientX === lastMouseX && e.clientY === lastMouseY) return;
                lastMouseX = e.clientX; lastMouseY = e.clientY;
                const item = e.target.closest('.halo-result-item');
                if (item) { const index = parseInt(item.getAttribute('data-index')); if (selectedIndex !== index) { selectedIndex = index; render(); } }
              };
        ...
        if (activeRule) {
          ...
          if (activeRule.label && !activeRule.hideLabel) {
            ...
            const flexPos = pos === 'center' ? '50%' : pos === 'left' ? '40px' : 'auto';
            const flexRight = pos === 'right' ? '40px' : 'auto';
            const transform = pos === 'center' ? 'translateX(-50%)' : 'none';
            
            // Professional charcoal-on-accent hanging tab
            css += `
              body::before { 
                content: "${activeRule.label}"; 
                position: fixed; top: 0; left: ${flexPos}; right: ${flexRight}; transform: ${transform};
                ...
                box-shadow: 0 8px 30px rgba(0,0,0,0.4), 0 0 15px ${color}44;
              }
            `;
          }
        ...
        revealHaloFields(); applyMandatoryGhosting(); setupCommandPalette();
      ...
      args: [rule, commands, themeConfig, vault]
    });
  ...
  if (msg.type === 'RELOAD_RULES') {
    ...
    await chrome.scripting.executeScript({
      ...
      func: () => {
        ...
        // Fill standard summary and text fields
        fill('input[name="summary"], #input-summary, #summary', 'UAT Magic Fill: All regression tests passed. Verified.');
        
        // Fill ALL rich text editors (Froala)
        const richTextVal = '1. Quiesce connections.<br>2. Apply patches.<br>3. Verify services.<br>4. Close change.';
        document.querySelectorAll('.fr-view').forEach(el => {
            el.innerHTML = richTextVal;
            el.dispatchEvent(new Event('input', { bubbles: true }));
        });

        // Fill other common custom field types and text areas
        document.querySelectorAll('textarea').forEach(el => {
           if (!el.value) {
             el.value = 'Magic Fill: Comprehensive testing data generated.';
             el.dispatchEvent(new Event('input', { bubbles: true }));
           }
        });
      }
     ...
     const results = await chrome.scripting.executeScript({
       ...
       func: () => {
         ...
         document.querySelectorAll('input:not([type="hidden"]), select, textarea').forEach(el => {
           const id = el.id || el.getAttribute('name') || el.getAttribute('data-field-name') || el.getAttribute('placeholder');
    ...
    // CRITICAL FIX: We must execute in the 'MAIN' world to access the page's 'monaco' variable
    await chrome.scripting.executeScript({
      target: { tabId: tab.id },
      world: 'MAIN',
      func: () => {
          ...
          if (window.monaco && window.monaco.editor) {
            ...
            if (models && models.length > 0) {
              ...
              if (currentSql && currentSql.trim()) {
                // Pushes to undo stack so user can CTRL+Z the formatting
                model.pushEditOperations([], [
                  { range: model.getFullModelRange(), text: prettifySql(currentSql) }
                ]);
              }
            }
          }
