...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Load Data
  const data = await chrome.storage.local.get(['rules', 'commands', 'vault', 'themeConfig']);
  rules = data.rules || [];
  ...
  const saveAll = async () => {
    await chrome.storage.local.set({ rules, commands, vault, themeConfig });
  ...
  vaultCaptureBtn.onclick = async () => {
    const name = vaultNameInput.value.trim() || `Snapshot ${new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`;
    ...
    importInput.onchange = (e) => {
      ...
      reader.onload = async (event) => {
        try {
          ...
          if (imported.vault) vault = imported.vault;
